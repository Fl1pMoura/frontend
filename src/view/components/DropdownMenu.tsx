...
function DropdownMenuContent({children, className, sideOffset, alignOffset} : {children: React.ReactNode, className?: string, sideOffset?: number, alignOffset?: number}){
  return(
    <RdxDropdownMenu.Portal>
      <RdxDropdownMenu.Content sideOffset={sideOffset} alignOffset={alignOffset} className={cn("z-50 bg-white rounded-2xl border-gray-100 border p-2 space-y-2 shadow-black/10 shadow-lg data-[side=bottom]:animate-slide-up-and-fade data-[side=top]:animate-slide-down-and-fade", className)}>
        {children}
